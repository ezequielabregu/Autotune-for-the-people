...
def download_file():
    return send_file(os.path.join(server_path,'uploads', 'output.wav'), as_attachment=True)
...
def autotune(y, sr):
    ...
    # 6) pitch detection
    f0, _, _ = librosa.pyin(
        y, frame_length=frame_length, hop_length=hop_length, sr=sr, fmin=fmin, fmax=fmax
    )
...
def main_at(audio_input):
    ...
    sf.write(os.path.join(server_path,'uploads', 'output.wav'), pitch_corrected_y, sr)
